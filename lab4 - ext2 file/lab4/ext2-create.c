...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group = 8*NUM_BLOCKS;
	superblock.s_frags_per_group = 8*NUM_BLOCKS;
...
void write_block_bitmap(int fd)
{
	...
	for (int block = 0; block < LAST_BLOCK; block++) {
		...
		// mask to set the bit of current index
		int bitMask = 1 << bitIndex;

		// apply the mask
		map_value[byteIndex] |= bitMask;
	}

	for (int block = BLOCK_SIZE-1; block < BLOCK_SIZE*8; block++) {
		int byteIndex = block / 8;
		int bitIndex = block % 8;
		// mask to set the bit of current index
		int bitMask = 1 << bitIndex;
...
void write_inode_bitmap(int fd)
{
	...
	for (int inode = 0; inode < LAST_INO; inode++) {
	...
	for (int inode = NUM_INODES; inode < BLOCK_SIZE*8; inode++) {
		int byteIndex = inode / 8;
		int bitIndex = inode % 8;
		// mask to set the bit of current index
		int bitMask = 1 << bitIndex;
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_blocks = 2; //TODO: not sure
	...
	hello_inode.i_blocks = 0; //TODO: not sure
	// hello_inode.i_block[0] = HELLO_INO;
...
void write_hello_world_file_block(int fd)
{
	...
	ssize_t string_size = 12;
	if (write(fd, hello_world, string_size) != string_size)
	{
		errno_exit("write");
	}
}
