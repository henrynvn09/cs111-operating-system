...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int code = pthread_mutex_init(&entry->lock, NULL);
		if (code != 0) {
                             ...
                             uint32_t value)
{
	...
	int code = pthread_mutex_lock(&hash_table_entry->lock);
	if (code != 0) {
	...
	if (list_entry != NULL) {
		...
		code = pthread_mutex_unlock(&hash_table_entry->lock);
		if (code != 0) {
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int code = pthread_mutex_destroy(&entry->lock);
		if (code != 0) {
